...
def data_partition(fname, path=None):
    ...
    if path == None:
        f = open('../../dataset/raw_data/%s.txt' % fname, 'r')
    else:
        f = open(path, 'r')
    for line in f:
...
def evaluate(model, dataset, args):
    ...
    for u in users:
        ...
        # 修改部分：计算 NDCG@10 和 Hit Ratio@10
        if rank < 10:  # 如果目标项在前 10 名内
            HT += 1
            NDCG += 1 / np.log2(rank + 2)  # rank + 2 是因为索引从 0 开始，且 log2(1) = 0
...
def evaluate_valid(model, dataset, args):
    ...
    valid_user = 0.0
    HT = 0.0
    # quick validation: sample at most 1000 users
    if usernum > 1000:
